      ...
      <div class="photo-grid">
        <template v-for="(photo, index) in currentPageData" :key="index">
          <div class="photo-item">
            <el-image
              :src="photo.bigPicUrl + '/minipreview'"
              :preview-src-list="[photo.bigPicUrl]"
              fit="contain"
            />
...
import { getTruingPhotos } from '@/api/pay/album'
...
const loadPhotos = async () => {
  try {
    const result = await getTruingPhotos(props.order.id)
    photos.value = result
  } catch (error) {
